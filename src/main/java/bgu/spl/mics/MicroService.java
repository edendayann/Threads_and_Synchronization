...
public abstract class MicroService implements Runnable {
    ...
    ConcurrentHashMap<Class<? extends Event<?>>, Callback<Message>> eventToCallBackHashMap;
    ConcurrentHashMap<Class<? extends Broadcast>, Callback<Message>> broadcastToCallBackHashMap;
    private MessageBusImpl messageBus; //TODO I think each MS need the have the Singleton MSB in order to function properly.
    //TODO because if not I dont see a way the MS can excess the MB functions.

    /**
     * @param name the micro-service name (used mainly for debugging purposes -
     *             does not have to be unique)
     */
    public MicroService(String name, MessageBusImpl messageBus) {
        this.name = name;
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        eventToCallBackHashMap.put(type, callback);
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        broadcastToCallBackHashMap.put(type, callback);
        ...
        while (!terminated) {
            try {
                ...
                if (message instanceof Event)
                    callback = eventToCallBackHashMap.get(message.getClass());
                else if(message instanceof Broadcast)
                    callback = broadcastToCallBackHashMap.get(message.getClass());
